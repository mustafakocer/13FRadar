...
export const LAZY_PAGES = {
  ...
  Checkout: { load: () => import('./Checkout.jsx'), match: /^\/checkout\/(success|cancel)$/ },
  Account: { load: () => import('./Account.jsx'), match: /^\/account$/ },
  Filers: { load: () => import('./Filers.jsx'), match: /^\/filers(\/|$)/ },
  ContentPage: { load: () => import('./ContentPage.jsx'), match: /^\/(?:(guides|rehber|compare|karsilastir)\/[a-z0-9-]+|privacy|terms|gizlilik|kullanim-sartlari)$/ },
